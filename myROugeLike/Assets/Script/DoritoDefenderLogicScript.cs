...
public class DoritoDefenderLogicScript : BaseEnemy
{
            ...
            if(Timer>ShootTime)
            {
                ...
                if (Bullets.Count > 0)
                {
                    for (int i = Bullets.Count-1; i > 0; i--)
                    {
                        ...
                        {
                            if (Vector3.Distance(Bullets[i].GetComponent<BulletMovementEnemy>().StartPos, Bullets[i].transform.position) > 19)
                            {
                                Destroy(Bullets[i]);
    ...
    void ShootBullet()
    {
        ...
        temp.GetComponent<BulletMovementEnemy>().Spawn(5,new Vector3(this.transform.position.x-1000, this.transform.position.y, this.transform.position.z   ));
        Bullets.Add(temp);
